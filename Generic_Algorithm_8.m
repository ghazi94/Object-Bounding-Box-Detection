function [bounding_box, non_upscaled_bb, processed_binary] = Generic_Algorithm_8 (mediumResizedImage, mediumGrayScale, originalGrayScale, extra_properties)
%% This function implements the composite masking algorithm %%
    ...
    binaryMaskBottomFringe = imdilate(binaryMaskBottomFringe, strel('line', 24, 0));
    binaryMaskBottomFringe = imerode(binaryMaskBottomFringe, strel('line', 12, 90));
    binaryMaskBottomFringe = imclearborder(binaryMaskBottomFringe,4);
    regionPropAreaThreshold = extra_properties.RegionPropAreaThresholdAlgo8;
    ...
    elapsed = toc;
    disp(strcat('Algorithm 8 finished in: ', string(elapsed)));
end